function S=residual_entropy(T,P,y)
% It returns the residual entropy (J/mol.K)
...
dX=sum(sum(dA));
Vig=R*T/(100*P); % Ideal gas volume (m3/kmol)
S=-R*log(Vig/(Vm-b))-(dX/(2*sqrt(2)*b))*...
    log((Vm+b*(1-sqrt(2)))/(Vm+b*(1+sqrt(2)))); % Residual entropy (kJ/kmol.K)
