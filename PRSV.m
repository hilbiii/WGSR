function [Vm,Zm]=PRSV(T,P,y)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% [Vm,Zm]=PR(T,P,y) retuns the mixture volume (Vm)in m3/kmol and the 
% mixture compressibility factor (Zm). It requires the system temperature
% (T) in K, the pressure (P) in bar and the molar fraction of the 
% components in the following order y=[H2,CO,CO2,H2O,N2,CH4].
% Ex: [Vm,Zm]=PRSV(550,1,[0.1,0.1,0.1,0.5,0.1,0.1])
% Algorithm based on:
...
coef=[1,-(1-B),(Ap-2*B-3*B^2),-(Ap*B-B^2-B^3)];% coefficients of the 
% PRSV-EoS writren as a polynomial function of the compressibility factor
Zm=real(max(roots(coef))); % Compressibility factor of the gas phase

Vm=Zm*R*T/(P*1e2); % m3/kmol
end
